...
```
ia-vacuum                    # dry-run: mostra o plano, não toca em nada
ia-vacuum --apagar           # executa exatamente o plano que o dry-run mostrou
ia-vacuum --incluir-hoje     # deixa recolher backup do próprio dia (ver política)
ia-vacuum --manter N         # cota de backups por família (padrão 8)
ia-vacuum --json             # plano machine-parseable
```
...
`iachat_core.py:422 rotate()` faz `nn = len(_recortes()) + 1` e grava com `write_text`
...
`post()` grava uma por nominado (`iachat_core.py:229`) e `marca_lida()` apaga ao ler
...
- **Idempotente.** A 2ª rodada não acha nada e diz por quê, com a data da anterior
  (`.vacuum.json`) — mesmo contrato do `rotate()` (`iachat_core.py:422`), que devolve
  `{"rodou": False, "motivo": ...}`.
...
`IACHAT_HOME` desvia a sala (`iachat_core.py:52`) e `HOME` desvia os diretórios de
backup — as duas costuras que permitem testar sem encostar em nada real. A bateria de
17/08 está na NOTA §5, com as duas saídas coladas.
